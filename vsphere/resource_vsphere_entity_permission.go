...
func resourceVsphereEntityPermission() *schema.Resource {
	...
	return &schema.Resource{
		Create:        resourceEntityPermissionCreate,
		Read:          resourceEntityPermissionRead,
		Update:        resourceEntityPermissionUpdate,
		Delete:        resourceEntityPermissionDelete,
		CustomizeDiff: resourceVSphereEntityPermissionCustomizeDiff,
		Schema:        sch,
	}
}

func resourceEntityPermissionCreate(d *schema.ResourceData, meta interface{}) error {
	log.Print("[DEBUG] Beginning create Permission")
	...
	if err != nil {
		return fmt.Errorf("error while creating permission %s", err)
	}
	d.SetId(d.Get("entity_id").(string))
	return resourceEntityPermissionRead(d, meta)
}

func resourceEntityPermissionRead(d *schema.ResourceData, meta interface{}) error {
	entityType := d.Get("entity_type").(string)
	entityId := d.Get("entity_id").(string)
	log.Printf("[DEBUG] : Reading vm entity permissions for entity id %s and type %s", entityId, entityType)
	client := meta.(*VSphereClient).vimClient
	authorizationManager := object.NewAuthorizationManager(client.Client)
	...
	if err != nil {
		return fmt.Errorf("error while reading permissions %s", err)
	}
	if len(permissionsArr) == 0 {
		log.Printf("[DEBUG] :the permissions for entity with id %s and type %s has been deleted", entityId, entityType)
		d.SetId("")
		return nil
	}
...
func resourceEntityPermissionUpdate(d *schema.ResourceData, meta interface{}) error {
	if d.HasChange("permissions") {
		oldPermissions, newPermissions := d.GetChange("permissions")
		log.Print("new perm ", newPermissions)
		log.Print("old perm ", oldPermissions)
		...
		for _, permission := range newPermissions.([]interface{}) {
			...
			i, err := strconv.ParseInt(permission.(map[string]interface{})["role_id"].(string), 10, 32)
			if err != nil {
				panic(err)
			}
			roleId := int32(i)
		...
		log.Print("new gp, ", usersAndGroups)
...
func resourceEntityPermissionDelete(d *schema.ResourceData, meta interface{}) error {
	...
	for _, permission := range permissions {
		...
		userOrGroup := permission.(map[string]interface{})["user_or_group"].(string)
		isGroup := permission.(map[string]interface{})["is_group"].(bool)
		log.Printf("Deleting user/group %s", userOrGroup)
		err = authorizationManager.RemoveEntityPermission(context.Background(), entityMor, userOrGroup, isGroup)
		if err != nil {
			return fmt.Errorf("error while deleting permission for the user/group %s %s", userOrGroup, err)
		}
...
func resourceVSphereEntityPermissionCustomizeDiff(d *schema.ResourceDiff, meta interface{}) error {
	if d.HasChange("entity_id") {
		oldEntityId, _ := d.GetChange("entity_id")
		if oldEntityId.(string) != "" {
			return fmt.Errorf("change in entity id is not allowed post creation")
		}
	}
	if d.HasChange("entity_type") {
		oldEntityType, _ := d.GetChange("entity_type")
		if oldEntityType.(string) != "" {
			return fmt.Errorf("change in entity type is not allowed post creation")
		}
	}
	return nil
}
